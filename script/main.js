...
import {searchInput, searchBtn, mainContainer, detailContainer, city, curDate, weatherStatus, mainWeatherIcon, mainTemp, shortTable, modal, todayNav, day5Nav, errorDiv, todayDiv, nearbyDiv, daysDiv, smallDaysDiv, bigDaysDiv} from "./htmlElements.js";
import {timeConvert, getDay, getDayFull, getDate, getMonth, getTime, temp} from "./convertors.js";
searchInput.value = "";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let crd = pos.coords;
    getApi(`https://api.openweathermap.org/data/2.5/weather?lat=${crd.latitude}&lon=${crd.longitude}&units=metric&appid=b998541107a89d374df4d04c07df017d`);
  }
  function error() {
    alert("Give permission to get your geolocation or input in search box your place!");
    getApi(`https://api.openweathermap.org/data/2.5/weather?q=Lutsk&units=metric&appid=b998541107a89d374df4d04c07df017d`);
  }
});
...
function searchLocation(){
  if (searchInput.value !=''){
    ...
    detailContainer.hidden = true;
    modal.hidden = false;
    getApi(`https://api.openweathermap.org/data/2.5/weather?q=${searchInput.value}&units=metric&appid=b998541107a89d374df4d04c07df017d`);
    searchInput.value = "";
  }
};
...
function getWeather(data) {
  getApi(`https://api.openweathermap.org/data/2.5/forecast?q=${data.name}&units=metric&cnt=40&appid=b998541107a89d374df4d04c07df017d`);
  getApi(`https://api.openweathermap.org/data/2.5/find?lat=${data.coord.lat}&lon=${data.coord.lon}&cnt=5&units=metric&appid=b998541107a89d374df4d04c07df017d`);
  curDate.innerHTML = `${getDate(new Date())} ${getMonth(new Date())} ${new Date().getFullYear()}`;
  ...
  weatherStatus.innerHTML = `${data.weather[0].description.replace(/\b\w/,(match) => match.toUpperCase())}`;
  ...
  mainTemp.innerHTML += `<p>Real Feel ${temp(data.main.feels_like)}<sup>o</sup> C</p>`;
  shortTable.innerHTML = `<tr><td>Sunrise:</td><td>${timeConvert(data.sys.sunrise)}</td></tr><tr><td>Sunset:</td><td>${timeConvert(data.sys.sunset)}</td></tr><tr><td>Duration:</td><td>${duration}</td></tr>`;
...
function getNearby(data) {
  ...
  data.forEach((element) => {
    ...
    h3.addEventListener('click', ()=>{
      getApi(`https://api.openweathermap.org/data/2.5/weather?q=${element.name}&units=metric&appid=b998541107a89d374df4d04c07df017d`);
    })
...
function getNextDay(data){
  ...
  smallDay.forEach(element => {
    ...
    div.append(h1);
    let p = document.createElement("p");
    p.innerHTML = `${element.weather[0].description.replace(/\b\w/, (match) =>match.toUpperCase())}`;
    div.append(p);
    div.addEventListener("click", ()=>{
      ...
      if(chooseDay.length>6) {chooseDay.length=6};
  ...
  if(todayMas.length>6) {todayMas.length=6};
...
function getBigDay (data, parentEl, selector){
  ...
  parentEl.append(div);
  const titleMas = ['',``,"Forecast","Temp (<sup>o</sup>C)","Real Feel","Wind (km/h)"];
  ...
  data.forEach(element => {
    const date = new Date(element.dt * 1000);
    document.querySelector(`.tr1${selector}`).innerHTML += `<td>${getDayFull(date)}<br>${getTime(date)}</td>`;
    document.querySelector(`.tr2${selector}`).innerHTML += `<td><img class="imgImg" src="http://openweathermap.org/img/wn/${element.weather[0].icon}@2x.png"></td>`;
    document.querySelector(`.tr3${selector}`).innerHTML += `<td>${element.weather[0].description.replace(/\b\w/, (match) =>match.toUpperCase())}</td>`;
    document.querySelector(`.tr4${selector}`).innerHTML += `<td>${temp(element.main.temp)}<sup>o</sup> C</td>`;
    document.querySelector(`.tr5${selector}`).innerHTML += `<td>${temp(element.main.feels_like)}<sup>o</sup> C</td>`;
    document.querySelector(`.tr6${selector}`).innerHTML += `<td>${(element.wind.speed * 3.6).toFixed(1)}</td>`;
  });
  if (data.length<=3){
